# In this code I have implemented Logistic Regression using Stochastic without Feature scaling.
import numpy as np
import pandas as pd
...
def stochastic_gradient_descent(x,y,learning_rate,iterations):
    ...
    for i in range(iterations):
            ...
            xi = x[random_index,0:3].reshape(1,x.shape[1])
            yi = y[random_index,0].reshape(1,1)
            
            prediction = hypothesis(theta,xi)
            loss=prediction-yi
...
# Finding parameters by Stochatic GDA
# Learning rate=0.01 , epochs=50000
w=stochastic_gradient_descent(train_x,train_y,0.01,50000)
# Finding accuracy
acc=accuracy(test_x,test_y,w)
print('The accuracy is: ', acc)
w=w.flatten()
# w=[-25.16131856, 0.20623159, 0.20147149]
