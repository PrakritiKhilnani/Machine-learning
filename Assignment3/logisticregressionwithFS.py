# In this code I have implemented logistic regression using batch gradient with feature scaling and found out the accuracy.
import numpy as np
import pandas as pd
...
B=x[:,2]
B_norm=(B-min(A))/(max(B)-min(B))
X_norm=np.c_[x[:,0],A_norm,B_norm]
...
# Finding parameters by Batch GDA
# Learning rate=0.01 , epochs=50000
w=batch_gradient_descent(train_x,train_y,1,500000)
# Finding accuracy
