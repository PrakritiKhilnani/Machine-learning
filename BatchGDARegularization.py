...
def batch_gradient_descent(X,Y,learning_rate,iterations,lambda_value):
     ...
     for i in range(0,iterations):
       # prediction = Hypothesis  
        ...
        loss=prediction-Y
        # regularization term
        reg_term = (lambda_value / (2 *len(X))) * np.sum(theta)
        cost_function= mean_squared_error(Y,prediction)+reg_term
        gradient=sum(np.dot(loss.T,X))
        # Updating the parameters i.e.theta here
        theta = (1-lambda_value/len(Y))*theta - learning_rate * gradient 
        print (f"Iteraton: {i},W={theta},mse={cost_function} ")              
     return theta,cost_function

W,J=batch_gradient_descent(train_x,train_y,0.0001,25,1000)
